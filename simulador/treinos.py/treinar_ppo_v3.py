...
class TermocicladorRealEnv(gym.Env):
    """
    Ambiente v3 — ação contínua (-1 a +1) e parâmetros calibrados com hardware real.
    Mudanças vs v2:
        - action_space: Discrete(3) → Box(-1, 1)  ← IA controla potência agora
        - Recompensa mais agressiva longe do alvo
    """

    def __init__(self):
        super().__init__()

        # MUDANÇA PRINCIPAL: ação contínua
    ...
    def step(self, action):
        ...
        # ação contínua — a IA decide a potência diretamente
        u_cmd = float(action[0])
        u_cmd = np.clip(u_cmd, -1.0, 1.0)
        self.last_u_cmd = u_cmd

        # canal PC → ESP com jitter
        ...
        # recompensa mais agressiva longe do alvo
        erro = abs(target - temp_real)
        reward = -erro
        if erro < 2.0:
            reward += 2.0   # bônus maior por precisão
        elif erro < 5.0:
            reward += 0.5
        elif erro > 20.0:
            reward -= 2.0   # penalidade extra se estiver muito longe
...
class ProgressCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        print(f"\n{'='*60}")
        print(f"TREINAMENTO PPO v3 — Ação contínua + parâmetros reais")
...
def main():
    ...
    env = Monitor(TermocicladorRealEnv())
    ...
    checkpoint_cb = CheckpointCallback(
        save_freq=100_000,
        save_path="modelos/",
        name_prefix="ppo_pcr_v3",
        verbose=1
    )

    print("Criando modelo PPO v3...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=0,
    ...
    model.save("ppo_pcr_v3_final")
    print("\n✅ Modelo salvo como 'ppo_pcr_v3_final.zip'")
