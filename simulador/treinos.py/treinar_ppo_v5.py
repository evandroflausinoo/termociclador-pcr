...
class TermocicladorRealEnvV5(gym.Env):
    """
    Ambiente v5 — Controle Antecipatório com Penalidade de Momentum.
    Elimina o Overshoot mantendo aceleração máxima longe da meta e frenagem suave perto dela.
    """
    ...
    def reset(self, seed=None, options=None):
        ...
        self.params = ThermalParams(Tamb=25.0, alpha=0.005, beta=0.4, dt=1.0, noise_std=0.3)
    ...
    def step(self, action):
        ...
        # ============================================================
        # RECOMPENSA v5 — FRENAGEM ANTECIPATÓRIA ANTI-OVERSHOOT
        # ============================================================
        erro_sinal = target - temp_real
        erro_abs = abs(erro_sinal)
        velocidade = temp_real - self.ultima_temp  # Taxa de variação (°C/s)

        # 1. Recompensa base proporcional ao erro
        reward = -erro_abs

        # 2. Bônus de Precisão no Patamar
        if erro_abs < 1.0:
            reward += 4.0   # Bônus máximo para erro < 1.0°C
        elif erro_abs < 2.5:
            reward += 2.0   # Bônus alto para erro < 2.5°C

        # 3. Penalidade por Alta Velocidade Próxima do Alvo (Anti-Overshoot)
        # Se estiver a menos de 6°C do alvo e se movendo muito rápido, é penalizado!
        if erro_abs < 6.0:
            velocidade_excessiva = abs(velocidade)
            if velocidade_excessiva > 0.3:  # Se estiver subindo/descendo a mais de 0.3°C/s perto da meta
                reward -= velocidade_excessiva * 2.5
...
class ProgressCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        print(f"\n{'='*60}")
        print(f"TREINAMENTO PPO v5 — Controle Antecipatório Anti-Overshoot")
        print(f"Estratégia: Aceleração total quando longe e frenagem perto da meta")
...
def main():
    ...
    env = Monitor(TermocicladorRealEnvV5())
    progress_cb = ProgressCallback(total_steps=TOTAL_STEPS)

    checkpoint_cb = CheckpointCallback(
        save_freq=100_000, save_path="modelos/", name_prefix="ppo_pcr_v5", verbose=1
    )

    print("Criando modelo PPO v5...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=0,
    ...
    model.save("modelos/ppo_pcr_v5_final")
    print("\n✅ Modelo salvo como 'modelos/ppo_pcr_v5_final.zip'")
